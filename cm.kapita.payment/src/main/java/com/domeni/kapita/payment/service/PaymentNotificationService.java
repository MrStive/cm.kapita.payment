...
public class PaymentNotificationService {
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "ProviderAttempt", notification.providerAttemptId().toString()));

    PaymentIntent paymentIntent =
        paymentIntentRepository
            .findById(new PaymentIntentId(providerAttempt.getPaymentIntentId().toUUID()))
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "PaymentIntent", providerAttempt.getPaymentIntentId().getValue()));
    ...
    return switch (status) {
      case "success" -> {
        if (paymentIntent.getStatus()
            == com.domeni.kapita.payment.domain.payment.PaymentIntentStatus.CREATED) {
          paymentIntent.markPaymentPending();
          providerAttempt.markPending("", "", "");
        }
      ...
      default ->
          throw new IllegalArgumentException("Unsupported provider payment status: " + status);
    };
  }
  ...
  private void verifyMoney(PaymentNotification notification, PaymentIntent paymentIntent) {
    ...
    if (!expectedCurrency.equalsIgnoreCase(notification.currency())) {
      throw new IllegalArgumentException("Payment notification currency does not match intent");
    }
  }
